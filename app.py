...
# ------------------------------
# Load model & dataset by default
# ------------------------------
@st.cache_resource
def load_model():
    return joblib.load("model.pkl")   # Pre-trained model file

@st.cache_data
def load_data():
    return pd.read_csv("telecom_churn.csv")  # Default dataset file
...
st.sidebar.title("📌 Navigation")
page = st.sidebar.radio("Go to", ["🏠 Overview", "📈 Data Insights", "🤖 Predict Churn", "📊 Model Performance"])
...
if page == "🏠 Overview":
    ...
    st.markdown("""
        Welcome to the **Telecom Churn Prediction App**.  
        This application helps telecom companies analyze customer data and predict **churn probability** using Machine Learning.  
        
        ### 🔑 Key Features:
        - Automated **churn prediction** with ML
        - 📊 Interactive dashboards & insights
        - 🧾 Model performance reports
        - 🎨 Beautiful, professional UI
    """)
    st.image("https://miro.medium.com/v2/resize:fit:1400/1*Vd8W7xRxXB0eZXxU6o5Ryw.png", caption="Telecom Churn Analysis", use_container_width=True)
...
# ------------------------------
    ...
    st.subheader("🔍 Churn Distribution")
    fig = px.pie(df, names="Churn", title="Customer Churn Distribution", color="Churn", hole=0.4)
    st.plotly_chart(fig, use_container_width=True)
    ...
    fig3 = px.histogram(df, x="PaymentMethod", color="Churn", barmode="group", title="Churn by Payment Method")
    st.plotly_chart(fig3, use_container_width=True)
...
# ------------------------------
    ...
    with col1:
        ...
        monthly_charges = st.slider("💰 Monthly Charges", 10, 150, 50)
        contract = st.selectbox("📜 Contract Type", ["Month-to-month", "One year", "Two year"])
    with col2:
    ...
    # Feature encoding (example, adapt to your preprocessing pipeline)
